...
data = carregar_dados()
# Zerar vendas e estoque apenas uma vez (primeira execução)
RESET_FLAG = "reset_done.flag"
if not os.path.exists(RESET_FLAG):
    data['sales'] = []
    data['stock'] = []
    salvar_dados(data)
    with open(RESET_FLAG, "w") as f:
        f.write("Reset concluído em " + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <select class="form-control" name="product_id" required id="productSelect">
                <option value=''>Selecione um produto</option>{product_options}
            </select>
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="product_code" id="productCode" placeholder="Digite o código">
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group">
            <label class="form-label">Quantidade</label>
            <input class="form-control" type="number" name="quantity" value="1" min="1" required id="quantityInput">
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group">
            <label class="form-label">Valor unitário (R$)</label>
            <input class="form-control" type="number" step="0.01" name="amount" required id="unitPrice">
        </div>
    </div>
</div>
